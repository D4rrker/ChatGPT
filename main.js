...
async function getIAResponse(textFromUser) {
  try {
    ...
    for await (const chunk of chuncks) {
      ...
      reply += content
      $textBot.innerHTML = marked(reply)
    ...
    console.error(error)
    addMessage(
      'Lo siento pero has llegó al los tokens máximos de la sesión.',
      'bot'
    )
  }
}
...
if ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window) {
  ...
  recognition.onerror = function (event) {
    ...
    if (error === 'no-speech') {
      $small.textContent = 'No se detecta ninguna voz: ' + event.error
    } else {
      $small.textContent =
        'Error en el reconocimiento de voz: ' +
        event.error +
        ' | Intenta probar en otro navegador como "Chrome"'
    }
  }

  recognition.onend = function () {
    $input.placeholder =
      'Escribe tu mensaje aquí o hazlo a través del micrófono.'
  }
